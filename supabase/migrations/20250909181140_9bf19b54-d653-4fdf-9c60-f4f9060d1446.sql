...
AS $$
  SELECT EXISTS (
    ...
    AND role IN (
      CASE required_role
        WHEN 'member' THEN ARRAY['member', 'moderator', 'admin', 'owner']::public.org_role[]
        WHEN 'moderator' THEN ARRAY['moderator', 'admin', 'owner']::public.org_role[]
        WHEN 'admin' THEN ARRAY['admin', 'owner']::public.org_role[]
        WHEN 'owner' THEN ARRAY['owner']::public.org_role[]
      END
    )
    AND is_active = true
  );
