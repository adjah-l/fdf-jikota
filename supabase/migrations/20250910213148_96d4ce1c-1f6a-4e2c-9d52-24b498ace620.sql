...
-- Permissive read policies for 5C data
CREATE POLICY IF NOT EXISTS "Organizations can manage their 5C settings" ON public.org_five_c_settings
FOR ALL USING (true) WITH CHECK (true);

CREATE POLICY IF NOT EXISTS "Organizations can manage their 5C events" ON public.five_c_events
FOR ALL USING (true) WITH CHECK (true);
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_org_five_c_settings_updated_at') THEN
